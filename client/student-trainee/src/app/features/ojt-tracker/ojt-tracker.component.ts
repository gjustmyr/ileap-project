...
export class OjtTrackerComponent implements OnInit {
    ...
    this.http.get(`${environment.apiUrl}/oeams/logs`, { headers }).subscribe({
      next: (response: any) => {
        if (response.status === 'success') {
          ...
          this.hoursCompleted = this.dailyLogs.reduce(
          ...
          const logsWithWarnings = this.dailyLogs.filter(
            (log) => log.validation_warning
          );
  ...
  calculatePresumedEndDate(): void {
    ...
    this.presumedEndDate.setDate(
      this.presumedEndDate.getDate() + totalCalendarDays
    );
