...
export class RequirementsComponent implements OnInit {
  ...
  postOjtRequirements: any[] = [
    ...
    },
    {
      ...
      description:
        'Evaluation report completed and submitted by your training supervisor',
      status: 'pending',
      file_url: null,
      validated: false,
      returned: false,
            ...
            .subscribe({
              next: (response: any) => {
                if (
                  ...
                  this.preOjtRequirements = this.preOjtRequirements.map(
                    (req) => {
                  ...
                  this.postOjtRequirements = this.postOjtRequirements.map(
                    (req) => {
                      const submission = submissionMap.get(req.id);
                      if (submission) {
                        return {
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          Swal.fire({
            ...
            title: 'Upload Failed',
            text:
              error.error?.detail || 'Failed to upload file. Please try again.',
